...
## Personal fork — Zaid (Ahmad Zaid)

This is my personal working fork of `hassancs91/claude-youtube-editor`. The original repo is the
upstream; this fork tracks my own changes for short-form content production (TikTok / marketplace
affiliate / YouTube Shorts / SEO tutorials).

**What I changed** (on top of the upstream):

- **`tools/gen_sfx.py`** — rewrote to use [Freesound API](https://freesound.org/) instead of
  ElevenLabs. Library still grows video-by-video, but no per-clip API cost.
- **`tools/mix_sfx.py`** — added `--no-sfx` flag. Per my audio policy I don't use background music;
  SFX pass is opt-in per video.
- **`tools/render_cuts.py`** + **`tools/make_proxy.py`** — swapped `h264_nvenc`/`hevc_nvenc` for
  `libx264`/`libx265`. I don't have an NVIDIA GPU on this machine.
- **`tools/gen_thumbnail_local.py`** *(new)* — local Pillow thumbnail composer (3 variants A/B/C,
  per-niche palette: affiliate / seo / ai-news / calm). Replaces Gemini Nano Banana Pro (no key
  required, $0 per render).
- **`tools/setup_recording.py`** *(new)* — pre-flight checker for test recordings: orientation,
  fps, codec, audio peak/RMS. Catches clipped/quiet audio before you bake.
- **`remotion/src/shots/zaid-templates/`** *(new)* — 4 portrait (1080×1920 @ 30fps) templates for
  short-form: `HookQuestion`, `ProductSpec`, `StepNumber`, `VerdictCard`.
- **`brand-zaid.md`** *(new)* — my personal brand extension: no-music policy, 3 niches
  (affiliate / SEO tutorial / AI news), recording setup notes.
- **`.gitignore`** — added `scratch/`, `analysis-*/`, `CHECKLIST_REKAMAN.md`, `*.mp4` so personal
  scratch work and reference files stay local.

**What I didn't touch** (and why): the Remotion kit (`remotion/src/lib/`), the pipeline contract
(`bake.py`, `render_cuts.py` core logic), the brand tokens (`remotion/src/brand.ts`). Those work.

Original author: [@hassancs91](https://github.com/hassancs91) ·
[YouTube: learnwithhasan](https://youtube.com/@learnwithhasan) ·
[Course: Build With AI](https://learnwithhasan.com/courses/build-with-ai-10/)
